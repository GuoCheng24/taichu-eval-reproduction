...
import os, sys, io, re, json, glob, time, argparse
...
ap.add_argument("--subset", type=int, default=0, help="evaluate a fixed random subset of this size (seed 0)")
...
IMG = f"{a.out}/img"; os.makedirs(IMG, exist_ok=True)
...
def load_items():
    ...
    if a.bench == "cvbench":
        ...
        for split in ["test_2d", "test_3d"]:
            ...
            for r in t:
                items.append({"id": f"{split}:{r['idx']}", "split": split, "task": r["task"], "source": r["source"],
                              "prompt": r["prompt"], "answer": r["answer"], "choices": r["choices"], "img": r["image"]["bytes"]})
    else:
        ...
        for r in t:
            items.append({"id": r["pid"], "prompt": r["query"], "answer": r["answer"], "choices": r["choices"],
                          "question_type": r["question_type"], "answer_type": r["answer_type"], "precision": r["precision"],
                          "img": r["decoded_image"]["bytes"]})
    if a.subset:
        import random; rng = random.Random(0); items = rng.sample(items, a.subset)
    return items[:a.limit] if a.limit else items

def letter_of(text):
    m = re.search(r"\(([A-F])\)", text) or re.search(r"\b([A-F])\b", text.strip()[:8]) or re.search(r"(?:answer|Answer)[^A-F]{0,20}\b([A-F])\b", text)
    return m.group(1) if m else None

def score_mathvista(it, text):
    text = text.strip(); final = text
    m = re.search(r"(?:answer|Answer)\s*(?:is|:)?\s*(.+)$", text, re.S)
    if m: final = m.group(1).strip()
    if it["question_type"] == "multi_choice":
        ch = it["choices"] or []
        # letter first, then exact choice text
        lm = re.search(r"\(([A-Z])\)", final) or re.search(r"^\s*([A-Z])\b", final)
        if lm and ord(lm.group(1)) - 65 < len(ch): pred = ch[ord(lm.group(1)) - 65]
        else:
    ...
    if it["answer_type"] == "list":
        ...
        return (norm(m2.group(0)) == norm(it["answer"])) if m2 else False, (m2.group(0) if m2 else final)
    nums = re.findall(r"-?\d+(?:\.\d+)?", final.replace(",", ""))
    if not nums: return False, final
    pred = nums[-1]
    try:
...
for klass in [cls] + [c for c in cls.__mro__ if hasattr(c, "_keys_to_ignore_on_load_unexpected")]:
    v = getattr(klass, "_keys_to_ignore_on_load_unexpected", None)
    if isinstance(v, list): klass._keys_to_ignore_on_load_unexpected = set(v)
...
for name, impl in ATTN.items():
    ...
    for k in ("_attn_implementation", "attn_implementation", "_attn_implementation_internal"):
        try: setattr(c, k, impl)
        except Exception: pass
model = cls.from_pretrained(MODEL, config=cfg, torch_dtype=torch.bfloat16, device_map="cuda:0", attn_implementation=ATTN).eval()
print("attn:", getattr(cfg, "_attn_implementation", None), {k: getattr(getattr(cfg, k, None), "_attn_implementation", None) for k in ("llm_config", "vision_config")}, flush=True)
MAXNEW = a.max_new or (32 if a.bench == "cvbench" else 1024)
t0 = time.time(); n = 0
with open(outf, "a") as fh:
    for it in items:
        path = f"{IMG}/{a.bench}_{it['id'].replace(':', '_')}.png"
        if not os.path.exists(path): Image.open(io.BytesIO(it["img"])).convert("RGB").save(path)
        messages = [{"role": "user", "content": [{"type": "image", "image": path}, {"type": "text", "text": it["prompt"]}]}]
        if a.thinking == "default":
            ...
            import importlib
            pv = importlib.import_module(type(processor).__module__.rsplit(".", 1)[0] + ".vision_utils").process_vision_info
            image_inputs, video_inputs, video_kwargs = pv(messages)
            prompt = processor.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True,
                                                             enable_thinking=(a.thinking == "on"))
            inputs = processor(text=[prompt], images=image_inputs, videos=video_inputs, return_tensors="pt", **(video_kwargs or {})).to(model.device)
        ...
        for tokstr in ("<|im_end|>", "<|endoftext|>", "<think>", "</think>"): final = final.replace(tokstr, "")
        final = final.strip() or text[-300:]
        ntok = int(gen.shape[1])
        if a.bench == "cvbench":
            pred = letter_of(final); gold = re.sub(r"[()]", "", it["answer"]).strip(); ok = pred == gold
            rec = {"id": it["id"], "split": it["split"], "task": it["task"], "source": it["source"], "gold": gold, "pred": pred, "ok": bool(ok),
                   "thought": thought, "ntok": ntok, "final": final, "text": text[-800:]}
        else:
            ok, pred = score_mathvista(it, final)
            rec = {"id": it["id"], "qtype": it["question_type"], "atype": it["answer_type"], "gold": str(it["answer"]), "pred": str(pred)[:80], "ok": bool(ok),
                   "thought": thought, "ntok": ntok, "final": final, "text": text[-800:]}
        fh.write(json.dumps(rec, ensure_ascii=False) + "\n"); fh.flush(); n += 1
        if n % 25 == 0 or n == len(items): print(f"  {n}/{len(items)}  {(time.time()-t0)/n:.1f}s/item  acc so far {sum(json.loads(l)['ok'] for l in open(outf))/max(1,sum(1 for _ in open(outf))):.3f}", flush=True)
